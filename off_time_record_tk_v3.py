...
def sort_data():
    # 提取所有项目
    items = data_list.get_children()
    # 获取每个项目的数据和对应的项目id
    data_with_id = [(data_list.item(item_id)['values'], item_id) for item_id in items]
    # 按日期排序，这里假设日期是每个项目数据第二个值
    sorted_data_with_id = sorted(data_with_id, key=lambda x: datetime.strptime(x[0][1], "%Y/%m/%d"), reverse=True)

    # 重新插入排序后的数据
    for index, (data, item_id) in enumerate(sorted_data_with_id):
        data_list.move(item_id, '', index)
